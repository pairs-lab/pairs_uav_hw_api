...
#include <pairs_msgs/msg/hw_api_rc_channels.hpp>
#include <pairs_msgs/msg/hw_api_rc_rssi.hpp>

#include <pairs_msgs/msg/rtk_gps.hpp>
#include <pairs_msgs/msg/rtk_fix_type.hpp>
#include <pairs_msgs/msg/gps_info.hpp>

#include <pairs_msgs/msg/hw_api_altitude.hpp>
...
typedef std::function<void(const pairs_msgs::msg::GpsInfo &msg)>                publishGNSSStatus_t;
typedef std::function<void(const pairs_msgs::msg::RtkGps &msg)>                 publishRTK_t;
typedef std::function<void(const pairs_msgs::msg::HwApiAltitude &msg)>          publishAltitude_t;
typedef std::function<void(const pairs_msgs::msg::Float64Stamped &msg)>         publishMagnetometerHeading_t;
typedef std::function<void(const sensor_msgs::msg::MagneticField &msg)>       publishMagneticField_t;
typedef std::function<void(const pairs_msgs::msg::HwApiStatus &msg)>            publishStatus_t;
typedef std::function<void(const pairs_msgs::msg::HwApiRcChannels &msg)>        publishRcChannels_t;
typedef std::function<void(const pairs_msgs::msg::HwApiRcRssi &msg)>            publishRcRssi_t;
 ...
 * The plugin can call these methods at will to provided data to the PAIRS UAV System.
...
struct Publishers_t
{
  ...
  /**
   * @brief Publisher for a general "odometry" message that can be used for vizualizing the state of the UAV, as produced by the flight controller. The PAIRS UAV
   * System will use this when running the "passthrough_ state estimator.
   */
  publishOdometry_t publishOdometry;

  /**
   * @brief Publisher for ground truth odometry which can be provided by, e.g., a simulator or a MoCap system. The PAIRS UAV System will use this when running the
